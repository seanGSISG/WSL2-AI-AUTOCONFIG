...
# ============================================================
# WSL2-AI-AUTOCONFIG - WSL2 AI Development Environment Setup
...
# Configuration
# ============================================================
WSL2AIAC_VERSION="0.1.0"
WSL2AIAC_REPO_OWNER="seanGSISG"
WSL2AIAC_REPO_NAME="WSL2-AI-AUTOCONFIG"
WSL2AIAC_REF="${WSL2AIAC_REF:-main}"
WSL2AIAC_RAW="https://raw.githubusercontent.com/${WSL2AIAC_REPO_OWNER}/${WSL2AIAC_REPO_NAME}/${WSL2AIAC_REF}"
export WSL2AIAC_RAW WSL2AIAC_VERSION
...
# Auto-detect user: prefer SUDO_USER when running as root, otherwise current user.
# Override with env vars: TARGET_USER=myuser TARGET_HOME=/home/myuser ...
if [[ -z "${TARGET_USER:-}" ]]; then
    if [[ $EUID -eq 0 ]] && [[ -n "${SUDO_USER:-}" ]]; then
        ...
        # Fallback for direct root login (typical VPS)
        TARGET_USER="ubuntu"
    fi
fi
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --skip-ubuntu-upgrade)
                ...
                ;;
            --update-checksums)
                # Maintenance: update checksums.yaml with current upstream values
...
detect_environment() {
    ...
    if [[ -f "$ACFS_LIB_DIR/os_detect.sh" ]]; then
        # shellcheck source=scripts/lib/os_detect.sh
        source "$ACFS_LIB_DIR/os_detect.sh"
    fi

    # Detect WSL2 environment for skipping VPS-specific features
    IS_WSL=false
    if grep -qi microsoft /proc/version 2>/dev/null; then
        IS_WSL=true
        log_detail "WSL2 environment detected - skipping VPS-specific features (except Tailscale)"
    fi
    export IS_WSL
...
init_target_paths() {
    # If running as TARGET_USER, use their home
    # If running as root, install for TARGET_USER
...
run_ubuntu_upgrade_phase() {
    ...
    # Skip in WSL2 - Ubuntu upgrades are managed by the host system
    if [[ "${IS_WSL:-false}" == "true" ]]; then
        log_detail "Skipping Ubuntu upgrade in WSL2 environment"
        return 0
    fi
...
normalize_user() {
    ...
    if [[ "$MODE" == "vibe" ]]; then
        local sudoers_file="/etc/sudoers.d/90-acfs-${TARGET_USER}"
        log_detail "Enabling passwordless sudo for $TARGET_USER"
        try_step_eval "Configuring passwordless sudo" \
            "echo '$TARGET_USER ALL=(ALL) NOPASSWD:ALL' | $SUDO tee '$sudoers_file' > /dev/null" || return 1
        try_step "Setting sudoers file permissions" $SUDO chmod 440 "$sudoers_file" || return 1
        if command_exists visudo && ! $SUDO visudo -c -f "$sudoers_file" >/dev/null 2>&1; then
            log_fatal "Invalid sudoers file generated at $sudoers_file"
        fi
        # Reset sudo cache to ensure new rules take effect immediately
        $SUDO -k 2>/dev/null || true
    fi

    # Ensure root's SSH keys are present for the target user (do not overwrite existing keys)
    # Skip in WSL2 - SSH keys are managed by the host system
    if [[ "${IS_WSL:-false}" == "true" ]]; then
        log_detail "Skipping SSH key migration in WSL2 environment"
    elif [[ $EUID -eq 0 ]] && [[ -f /root/.ssh/authorized_keys ]]; then
...
main() {
    ...
    # Handle --update-checksums: fetch current checksums and update checksums.yaml
    if [[ "$UPDATE_CHECKSUMS" == "true" ]]; then
        log_step "Updating checksums.yaml with current upstream values..."
        local checksums_file="${SCRIPT_DIR:-$(pwd)}/checksums.yaml"
        if [[ ! -f "$checksums_file" ]]; then
            log_fatal "checksums.yaml not found at $checksums_file"
        fi
        if [[ ! -w "$checksums_file" ]]; then
            log_fatal "Cannot write to $checksums_file"
        fi
        if type -t print_current_checksums &>/dev/null; then
            print_current_checksums > "$checksums_file"
            log_success "Updated checksums: $checksums_file"
        else
            log_fatal "print_current_checksums function not available (security.sh not loaded?)"
        fi
        exit 0
    fi
