...
if [[ -f "$HOME/.wsl2aiac/VERSION" ]]; then
    ACFS_VERSION="$(cat "$HOME/.wsl2aiac/VERSION" 2>/dev/null || echo "$ACFS_VERSION")"
...
if [[ -z "${ACFS_MODE:-}" ]] && [[ -f "$HOME/.wsl2aiac/state.json" ]]; then
    if command -v jq &>/dev/null; then
        ACFS_MODE="$(jq -r '.mode // empty' "$HOME/.wsl2aiac/state.json" 2>/dev/null || true)"
    fi
    if [[ -z "${ACFS_MODE:-}" ]]; then
        ACFS_MODE="$(sed -n 's/.*"mode"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$HOME/.wsl2aiac/state.json" | head -n 1)"
    fi
...
if [[ -z "${ACFS_TARGET_USER:-}" ]] && [[ -f "$HOME/.wsl2aiac/state.json" ]]; then
    if command -v jq &>/dev/null; then
        ACFS_TARGET_USER="$(jq -r '.target_user // empty' "$HOME/.wsl2aiac/state.json" 2>/dev/null || true)"
    fi
    if [[ -z "${ACFS_TARGET_USER:-}" ]]; then
        ACFS_TARGET_USER="$(sed -n 's/.*"target_user"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$HOME/.wsl2aiac/state.json" | head -n 1)"
    fi
fi
...
elif [[ -f "$HOME/.wsl2aiac/scripts/lib/gum_ui.sh" ]]; then
    source "$HOME/.wsl2aiac/scripts/lib/gum_ui.sh"
fi
...
resolve_session_lib() {
    if [[ -f "$HOME/.wsl2aiac/scripts/lib/session.sh" ]]; then
        echo "$HOME/.wsl2aiac/scripts/lib/session.sh"
        return 0
    fi
...
check_agents() {
    ...
    if grep -q "^alias ccd=" ~/.wsl2aiac/zsh/wsl2aiac.zshrc 2>/dev/null; then
        check "agent.alias.ccd" "ccd alias" "pass"
    else
        check "agent.alias.ccd" "ccd alias" "warn" "not in zshrc"
    fi

    if grep -q "^alias cod=" ~/.wsl2aiac/zsh/wsl2aiac.zshrc 2>/dev/null; then
        check "agent.alias.cod" "cod alias" "pass"
    else
        check "agent.alias.cod" "cod alias" "warn" "not in zshrc"
    fi

    if grep -q "^alias gmi=" ~/.wsl2aiac/zsh/wsl2aiac.zshrc 2>/dev/null; then
...
# Check cloud tools (removed in WSL2-AI-AUTOCONFIG)
check_cloud() {
    # Cloud tools not included in this stripped-down version
    return 0
}
...
load_skipped_tools() {
    SKIPPED_TOOLS_DATA=()
    local state_file="$HOME/.wsl2aiac/state.json"
...
# Deep check: Cloud CLI authentication (removed in WSL2-AI-AUTOCONFIG)
deep_check_cloud() {
    # Cloud tools not included in this stripped-down version
    # Only check GitHub auth which is still useful
    check_gh_auth
}
...
main() {
    ...
    case "$subcmd" in
        ...
        info|i)
            shift
            local info_script=""
            if [[ -f "$HOME/.wsl2aiac/scripts/lib/info.sh" ]]; then
                info_script="$HOME/.wsl2aiac/scripts/lib/info.sh"
        ...
        dashboard)
            ...
            if [[ -f "$HOME/.wsl2aiac/scripts/lib/dashboard.sh" ]]; then
                dashboard_script="$HOME/.wsl2aiac/scripts/lib/dashboard.sh"
        ...
        continue|progress)
            ...
            if [[ -f "$HOME/.wsl2aiac/scripts/lib/continue.sh" ]]; then
                continue_script="$HOME/.wsl2aiac/scripts/lib/continue.sh"
        ...
        cheatsheet|cs)
            ...
            if [[ -f "$HOME/.wsl2aiac/scripts/lib/cheatsheet.sh" ]]; then
                cheatsheet_script="$HOME/.wsl2aiac/scripts/lib/cheatsheet.sh"
        ...
        update)
            shift
            local update_script=""
            if [[ -f "$HOME/.wsl2aiac/scripts/lib/update.sh" ]]; then
                update_script="$HOME/.wsl2aiac/scripts/lib/update.sh"
        ...
        services-setup|services|setup)
            ...
            if [[ -f "$HOME/.wsl2aiac/scripts/services-setup.sh" ]]; then
                services_script="$HOME/.wsl2aiac/scripts/services-setup.sh"
        ...
        version|-v|--version)
            local version_file=""
            if [[ -f "$HOME/.wsl2aiac/VERSION" ]]; then
                version_file="$HOME/.wsl2aiac/VERSION"
            elif [[ -f "$SCRIPT_DIR/../VERSION" ]]; then
