...
# ~/.wsl2aiac/zsh/wsl2aiac.zshrc
# WSL2-AI-AUTOCONFIG canonical zsh config (managed). Safe, fast, minimal duplication.
#
...
add-zsh-hook chpwd _wsl2aiac_ls_after_cd
...
# --- WSL2AIAC env shim (optional) ---
[[ -f "$HOME/.local/bin/env" ]] && source "$HOME/.local/bin/env"

# --- WSL2AIAC CLI ---
# Provides `wsl2aiac <subcommand>` for post-install utilities
wsl2aiac() {
  local wsl2aiac_home="${WSL2AIAC_HOME:-$HOME/.wsl2aiac}"
  local wsl2aiac_bin="$HOME/.local/bin/wsl2aiac"
  local cmd="${1:-help}"
  shift 1 2>/dev/null || true

  case "$cmd" in
    services-setup|services|setup)
      if [[ -f "$wsl2aiac_home/scripts/services-setup.sh" ]]; then
        bash "$wsl2aiac_home/scripts/services-setup.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" services-setup "$@"
      else
        echo "Error: services-setup.sh not found"
        echo "Re-run the WSL2-AI-AUTOCONFIG installer to get the latest scripts"
        return 1
      fi
      ;;
    doctor|check)
      if [[ -f "$wsl2aiac_home/scripts/lib/doctor.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/doctor.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" doctor "$@"
      else
    ...
    session|sessions)
      if [[ -f "$wsl2aiac_home/scripts/lib/doctor.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/doctor.sh" session "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" session "$@"
      else
        echo "Error: doctor.sh not found"
        echo "Re-run the WSL2-AI-AUTOCONFIG installer to get the latest scripts"
        return 1
      ...
      ;;
    update)
      if [[ -f "$wsl2aiac_home/scripts/lib/update.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/update.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" update "$@"
      else
    ...
    continue|status|progress)
      if [[ -f "$wsl2aiac_home/scripts/lib/continue.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/continue.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" continue "$@"
      else
      ...
      ;;
    info|i)
      if [[ -f "$wsl2aiac_home/scripts/lib/info.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/info.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" info "$@"
      else
      ...
      ;;
    cheatsheet|cs)
      if [[ -f "$wsl2aiac_home/scripts/lib/cheatsheet.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/cheatsheet.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" cheatsheet "$@"
      else
      ...
      ;;
    dashboard|dash)
      if [[ -f "$wsl2aiac_home/scripts/lib/dashboard.sh" ]]; then
        bash "$wsl2aiac_home/scripts/lib/dashboard.sh" "$@"
      elif [[ -x "$wsl2aiac_bin" ]]; then
        "$wsl2aiac_bin" dashboard "$@"
      else
      ...
      ;;
    version|-v|--version)
      if [[ -f "$wsl2aiac_home/VERSION" ]]; then
        cat "$wsl2aiac_home/VERSION"
      else
        echo "WSL2AIAC version unknown"
      fi
      ;;
    help|-h|--help|*)
      echo "WSL2-AI-AUTOCONFIG - WSL2 AI Development Environment"
      echo ""
      echo "Usage: wsl2aiac <command>"
      echo ""
      echo "Commands:"
      ...
      echo "  continue        View installation progress"
